...
module.exports = function(grunt) {
	...
	grunt.task.registerTask('init', function(folder, filename) {
		var json = grunt.file.read(filename + '/images/' + filename + '_atlas_.json');
		json = json.replace('images/', '');
		json = json.replace('_atlas_', '');
		grunt.config.set('mkdir.all.options.create.0', 'banner_' + filename);
		grunt.config.set('copy.files.src.0', filename + '.html');
		grunt.config.set('copy.files.src.1', filename + '.js');
		grunt.config.set('copy.files.dest', 'banner_' + filename + '/');
		grunt.config.set('copy.image.dest', 'banner_' + filename + '/');
		grunt.config.set('rename.png.src', 'banner_' + filename + '/' + filename + '_atlas_.png');
		grunt.config.set('rename.png.dest', 'banner_' + filename + '/' + filename + '.png');
		grunt.config.set('replace.html.src', 'banner_' + filename + '/' + filename + '.html');
		grunt.config.set('replace.pixel.src', 'banner_' + filename + '/' + filename + '.html');
